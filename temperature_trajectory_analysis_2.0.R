...
#load and join hosp and patient
hosp <- read_data(paste0(tables_location, "clif_hospitalization", file_type)) %>% 
  distinct() 

patient <- read_data(paste0(tables_location, "clif_patient", file_type)) %>%
  distinct()
...
ventilator <- read_data(paste0(tables_location, "clif_respiratory_support", file_type)) %>% 
  filter(str_to_lower(device_category) == "imv")%>%
  distinct()%>%
  select(hospitalization_id)%>%
  deframe()

#load Locations 
adt <- read_data(paste0(tables_location, "clif_adt", file_type)) %>%
...
#Making sure recorded_dttm does not have time zones (SKIP IF NOT NEEDED)
merged_data <- merged_data %>%
  mutate(recorded_dttm = as.POSIXct(recorded_dttm, format = "%Y-%m-%dT%H:%M:%OS", tz = "UTC"))%>%
  mutate(recorded_dttm = format(recorded_dttm, format = "%Y-%m-%d %H:%M:%S"))%>%
  distinct()
###################### Table functions ########################################
med = function(v)
{
...
temp_algorithm <- merged_data %>%
  ...
  mutate(
    ...
    temp_time = as.POSIXct(recorded_dttm, format = "%Y-%m-%d %H:%M:%S"),
    in_dttm = as.POSIXct(in_dttm, format = "%Y-%m-%d %H:%M:%S")
  ) %>%
  ...
  select(hospitalization_id, hours_from_adm, temperature) %>%
  mutate(hours_from_adm = round(hours_from_adm, digits = 0)) %>%
  group_by(hospitalization_id, hours_from_adm) %>%
  ...
  )%>% select(hospitalization_id, model) 
...
table_alg <- temp_algorithm %>%
  left_join(hosp_patient) %>%
  filter(!is.na(age_at_admission)) %>%
  mutate(vent = ifelse(hospitalization_id %in% ventilator, 1, 0)) %>%
  mutate(icu = ifelse(hospitalization_id %in% icu_encounters, 1, 0)) %>%
  mutate(death = ifelse(discharge_category %in% c("Expired","expired"), 1, 0)) %>% 
...
table<- table_alg%>%
  inner_join(merged_data, by = "hospitalization_id")%>%
  rename(temperature = vital_value) %>%
  mutate(
    temp_time = as.POSIXct(recorded_dttm, format = "%Y-%m-%d %H:%M:%S"),
    in_dttm = as.POSIXct(in_dttm, format = "%Y-%m-%d %H:%M:%S")
...
table_icu<- table%>%
  inner_join(table_alg, by = "hospitalization_id")
...
table_temp_traj_cohort <- table_icu %>%
  ...
  summarise(
    ...
    n = n_distinct(hospitalization_id),
    .groups = 'drop'
  )%>%
  filter(hour != 73)
...
table2_alg_summary <- temp_algorithm %>%
  ...
  summarise(N = sum(n), 
            ...
            "Sex, male" = counts(sex_category == "Male"),
            Race = "",
            Black = counts(race_category == "Black or African American"), # check your corresponding string 
            White = counts(race_category == "White"),
            Other = counts(race_category != "Black or African American" & race_category != "White"),
            ICU = counts(icu),
...
output[4] = round(chisq.test(table_alg$model, table_alg$sex_category)$p.value, digits=3)
output[5] = round(chisq.test(table_alg$model, table_alg$race_category)$p.value, digits=3)
output[6] = NA
output[7] = NA
output[8] = NA
...
df_alg <- as.data.frame(summary(glm(vent ~ HSR + HFR + HT +  age_at_admission + sex_category + factor(race_category) + ethnicity_category, data = table_alg, family = "binomial"))$coefficients)%>%
  remove_rownames()%>%
  mutate(outcome = "Mechanical ventilation")

df_alg <- df_alg %>%
  rbind(as.data.frame(summary(glm(death ~ HSR + HFR + HT +  age_at_admission + sex_category + factor(race_category) + ethnicity_category, data = table_alg, family = "binomial"))$coefficients)%>%
          remove_rownames()%>%
          mutate(outcome = "Mortality"))

summary(glm(vent ~ HSR + HFR + HT + age_at_admission + sex_category + factor(race_category) + ethnicity_category, data = table_alg, family = "binomial"))$coefficients
