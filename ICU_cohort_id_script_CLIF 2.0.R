packages <- c("jsonlite", "duckdb", "lubridate", "tidyverse", "dplyr","table1",
              'rvest', "readr", "arrow", "fst", "lightgbm", "caret", 
              "Metrics", "ROCR", "pROC")
...
if (!dir.exists("output/intermediate")) {
  dir.create("output/intermediate")
}
...
# labs <- read_data(paste0(tables_location, "/clif_labs", file_type))
...
write.csv(icu_data, paste0( "output/intermediate/ICU_cohort", '.csv'), row.names = FALSE)
